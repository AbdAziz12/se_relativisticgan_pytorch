...
class SimpleGenerator(nn.Module):
    def __init__(self, input_channels=1, output_channels=1, base_filters=8):
        ...
        self.enc4 = self._conv(base_filters * 4, base_filters * 8, 15, 2)
    ...
    def _res_block(self, in_c, k):
        return nn.Sequential(
            # nn.Conv1d(channels, channels, k, s, k // 2),
            nn.Conv1d(in_c, in_c * 2, 1),       # Expansion: Perbanyak fitur
            nn.PReLU(),
            nn.Conv1d(in_c * 2, in_c * 2, k, 1, k // 2, groups=in_c * 2), # Depthwise
            nn.PReLU(),
...
class SimpleDiscriminator(nn.Module):
    def __init__(self, input_channels=2, base_filters=8):
        ...
        self.conv4 = self._conv(base_filters * 4, base_filters * 8, 15, 2)
